...
export const update = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const currentMember = await ctx.db.query("members").withIndex("by_workspace_id_user_id",
      (q) => q.eq("workspaceId", member.workspaceId).eq("userId", userId)).unique()
...
export const remove = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const [messages, reactions, conversations] = await Promise.all([
      ...
      ctx.db
        .query("conversations")
        .filter((q) => q.or(q.eq(q.field("memberOneId"), member._id),
